...
rimLight2.position.set(-10, 0, -2);
scene.add(rimLight2);

const rimLight3 = new THREE.DirectionalLight(0xffffff, 0.3);
rimLight3.position.set(1 -2, 5);
scene.add(rimLight3);
...
// Intenso package model with textures
const loader = new GLTFLoader();
const intensoTextureBase = new THREE.TextureLoader().load('./images/intenso_packaging_texture.png', (texture) => {
  texture.flipY = false;  
...
const intensoTextureNorm = new THREE.TextureLoader().load('./images/intenso_packaging_texture_normal.png', (texture) => {
  texture.flipY = false;  
  texture.wrapS = THREE.RepeatWrapping;
  texture.wrapT = THREE.RepeatWrapping;
});
...
// Load first model
loader.load('./models/Intenso_capsule_packaging.glb', (gltf) => {
  intenso = gltf.scene;
  
  
  const intensoScale = 2 / new THREE.Box3().setFromObject(intenso).getSize(new THREE.Vector3()).length();
  intenso.scale.setScalar(intensoScale);

  // Process first model materials
  intenso.traverse((child) => {
    if (child.isMesh) {
      intensoMaterials.push(child.material);
      child.material = new THREE.MeshStandardMaterial({
        map: intensoTextureBase,
        normalMap: intensoTextureNorm,
        roughness: 0.4,
        metalness: 0.7,
        transparent: true,   
        opacity: 0     
  ...
  intenso.position.set(-2, -4, 0);
  intenso.scale.set(2.6, 2.6, 2.6); 

  let startingAngle = intenso.rotation.y;
  ...
  const deviceInfo = detectMobileDevice();
    ...
    switch (deviceInfo.deviceType) {
        case 'small-mobile':
            ...
            intenso.scale.set(2, 2, 2);
            intenso.rotation.set(0, 0, 0);
            intenso.position.set(-1, -2, 0);
            break;
        ...
        case 'large-mobile':
            ...
            intenso.scale.set(2.4, 2.4, 2.4);
            intenso.rotation.set(0, 0, 0);
            intenso.position.set(1.4, -10, 0);
            break;
...
let intensoBoxHelper, packetModelBoxHelper;

function createBoxHelpers() {

  // Create new helpers
  if (intenso) {
    intensoBoxHelper = new THREE.BoxHelper(intenso, 0xffff00); 
    scene.add(intensoBoxHelper);
  }
...
function animate() {
  ...
  if (intenso) {
    //intenso.rotation.z = Math.sin(clock.elapsedTime * 1.0) * 0.08;
    //intenso.rotation.y += 0.06 * delta;
    intenso.rotation.y = startingAngle + Math.sin(clock.elapsedTime * rotationSpeed) * rotationRange;
  }
